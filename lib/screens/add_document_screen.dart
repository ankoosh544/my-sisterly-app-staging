...
class AddDocumentScreenState extends State<AddDocumentScreen> {
  ...
  _upload() {
    if (_documentNumberText.text.isEmpty) {
      ApiManager.showFreeErrorToast(
          context, "Il numero documento è obbligatorio");
      return;
    }

    if (_documentExpirationDate == null) {
      ApiManager.showFreeErrorToast(
          context, "La data di scadenza documento è obbligatoria");
      return;
    }

    if (_fiscalCodeText.text.isEmpty) {
      ApiManager.showFreeErrorToast(
          context, "Il codice fiscale è obbligatorio");
      return;
    }

    if (_frontDocumentImage == null) {
      ApiManager.showFreeErrorToast(
          context, "L'immagine fronte del documento è obbligatoria");
      return;
    }
    ...
    var params = {
      ...
      "expiration_date":
          DateFormat("yyyy-MM-dd").format(_documentExpirationDate!)
    };
    ApiManager(context).makeUploadDocumentsRequest(
        context,
        "POST",
        '/client/document',
        _frontDocumentImage,
  ...
  saveProfile() async {
    ...
    var params = {"identity_code": _fiscalCodeText.text};
                      ...
                      InkWell(
                        onTap: () async {
                          ...
                          final DateTime? picked = await showDatePicker(
                            context: context,
                            initialDate:
                                _documentExpirationDate ?? DateTime.now(),
                            firstDate:
                      ...
                      Container(
                          ...
                          child: Column(children: [
                            ...
                            TextButton(
                              ...
                              onPressed: () async {
                                ...
                                if (source == ImageSource.camera) {
                                  _frontDocumentImage = (await picker.pickImage(
                            ...
                            if (_frontDocumentImage != null)
                              ClipRRect(
                                child:
                                    Image.file(File(_frontDocumentImage!.path)),
                                borderRadius: BorderRadius.circular(12),
                              ),
                            SizedBox(height: 10)
                          ])),
                          ...
                          child: Column(children: [
                            ...
                            TextButton(
                              ...
                              onPressed: () async {
                                ...
                                if (source == ImageSource.camera) {
                                  _backDocumentImage = (await picker.pickImage(
                                      source: ImageSource.camera))!;
                                } else {
                                  _backDocumentImage = (await picker.pickImage(
                                      source: ImageSource.gallery))!;
                            ...
                            if (_backDocumentImage != null)
                              ClipRRect(
                                child:
                                    Image.file(File(_backDocumentImage!.path)),
                                borderRadius: BorderRadius.circular(12),
                              ),
                            SizedBox(height: 10)
                          ])),
